...
export class MB3DFormulaPanel extends HTMLElement {
  private slots: { formula: string; iterations: number }[] = [];
  ...
  private renderSlots() {
    ...
    container.querySelectorAll('select').forEach(sel => {
      sel.addEventListener('change', (e) => {
        ...
        this.slots[idx].formula = target.value;
        this.renderSlots();
        this.emitChange();
      });
    });
    ...
    container.querySelectorAll('input[type="number"]').forEach(inp => {
      inp.addEventListener('change', (e) => {
        ...
        const idx = parseInt(target.dataset.slot!);
        this.slots[idx].iterations = parseInt(target.value);
        this.emitChange();
      });
    });
  }
  ...
  private emitChange() {
    const hybridMode = (this.shadowRoot!.querySelector('input[name="hybrid"]:checked') as HTMLInputElement)?.value;
    this.dispatchEvent(new CustomEvent('mb3d-formula-change', {
      bubbles: true, composed: true,
      detail: { slots: [...this.slots], hybridMode },
    }));
  }

  /** Set formula configuration from a loaded header. */
  setConfig(slots: { formula: string; iterations: number }[], hybridMode: string) {
    this.slots = slots;
    this.renderSlots();
    const radio = this.shadowRoot!.querySelector(`input[value="${hybridMode}"]`) as HTMLInputElement | null;
