...
template.innerHTML = `
  <style>
    ...
    legend { font-weight: 600; font-size: 11px; color: var(--mb3d-accent, #4a9eff); }
    label { display: flex; align-items: center; gap: 4px; margin: 4px 0; }
    label span { min-width: 80px; color: var(--mb3d-text-dim, #888); }
    input[type="number"], input[type="text"] {
      flex: 1; background: #1a1a1a; border: 1px solid var(--mb3d-border, #404040);
      border-radius: 2px; color: var(--mb3d-text, #e0e0e0); padding: 2px 4px;
...
export class MB3DControls extends HTMLElement {
  ...
  connectedCallback() {
    this.shadowRoot.querySelectorAll('input').forEach((input) => {
      input.addEventListener('change', () => {
        this.dispatchEvent(
          new CustomEvent('mb3d-param-change', {
            bubbles: true,
            composed: true,
   ...
   */
  getParams() {
    const q = (id) => this.shadowRoot.querySelector(`#${id}`).value;
    return {
